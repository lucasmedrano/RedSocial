...
def centralidad(grafo_marvel, cantidad):
    ...
    recorrido = grafo_marvel.random_walk(5000, None, True)
    cant_rep_personajes = Counter(recorrido)    
    similares = cant_rep_personajes.most_common()
...
def main():
    ...
    while(True):
        comando = input("")
        if comando == '': break   #Vamos a tener que separar los parametros por ',' y despues separar el comando que quiere correr
        lista = comando.split()     #con nombres de mas de dos palabras falla. Tambien pasar a enteros los que se tienenq ue recibir como enteros
